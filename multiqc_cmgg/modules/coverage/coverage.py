...
def genstats_cov_thresholds(
    cum_fraction_by_cov: Dict[int, float], threshs: List[int]
) -> Dict[str, float]:
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        genstats_by_sample: Dict[str, Dict[str, Union[int, float]]] = defaultdict(
            dict
        )  # mean coverage
        ...
        list_yaml_configs = [
            "show_hide_buttons",
            "show_hide_mode",
            "show_hide_patterns,show_hide_color",
        ]
        ...
        for f in self.find_log_files(
            f"coverage/region_dist", filecontents=False, filehandles=False
        ):
            ...
            if "_" not in s_name and s_name in data_dicts_region:
                ...
                if (
                    data_dicts_region[s_name][f"{str(thresh)}_x_pc"] >= 90
                    and more_less90[name] != "#d9534f"
                ):
    ...
    def parse_cov_dist(
        ...
        for f in self.find_log_files(
            ...
            genstats_by_sample[s_name]["median_coverage"] = calc_median_coverage(
                cum_fraction_by_cov
            )
